...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("(auto|manual)\\s+(black|white)", this::doMA),
        new Command("[a-jA-J]\\d{1,2}", this::doPMove)
    };
    ...
    /** Make a move.
     * @param mat Matcher for the move.*/
    private void doMove(Matcher mat) {
        int addon = (mat.group(1) == null ? 3 : 0);
        Square from = Square.sq(mat.group(1 + addon));
        Move m = Move.mv(mat.group(0));
        if (m == null || !board().isLegal(m)
                || !board().turn().equals(board().get(from))) {
    ...
    /** Temporarily store the moves for doMove().
     * @param mat string representing single square.*/
    private void doPMove(Matcher mat) {
        String temp = mat.group(0);
        _moves += temp;
        if (_moves.length() >= 8) {
            Matcher move = Move.MOVE_PATTERN.matcher(_moves);
            _moves = "";
            if (move.matches()) {
                doMove(move);
            }
    ...
    /** Assign AI or manual player to a color.
     * @param mat Matcher for the manual/auto switch.*/
    private void doMA(Matcher mat) {
        Player mora = (mat.group(1).equals("manual")
                ? _manualPlayerTemplate : _autoPlayerTemplate);
        String player = mat.group(2);
        if (player.equals("black")) {
            _black = mora.create(BLACK, this);
        } else {
            _white = mora.create(WHITE, this);
        }
    }
    ...
    /** Temporary storage of clicked squares. */
