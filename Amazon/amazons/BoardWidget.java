...
class BoardWidget extends Pad {
    ...
    static final Color
        ...
        MOVE_PICKING_COLOR = new Color(225, 22, 10);
    ...
    private void drawGrid(Graphics2D g) {
        ...
        for (int col = 0; col <= 9; col += 1) {
            for (int row = 0; row <= 9; row += 1) {
                ...
                Color color = (row + col) % 2 == 1
                        ? LIGHT_SQUARE_COLOR : DARK_SQUARE_COLOR;
                g.setColor(color);
                g.fillRect(cx(curr), cy(curr),
    ...
    public synchronized void paintComponent(Graphics2D g) {
        ...
        for (int col = 0; col <= 9; col += 1) {
            for (int row = 0; row <= 9; row += 1) {
                Square curr = Square.sq(col, row);
                Piece p = _board.get(curr);
                if (p.equals(WHITE) || p.equals(BLACK)) {
                    drawQueen(g, curr, p);
                ...
                if (curr.equals(_clicked)) {
                    g.setColor(MOVE_PICKING_COLOR);
                    g.fillRect(cx(curr), cy(curr),
                            SQUARE_SIDE, SQUARE_SIDE);
                }
            }
    ...
    /** The square currently being selected. */
